...
- `agent-authbridge-agent-aud` scope (realm default - adds Agent's SPIFFE ID to all tokens)
...
This flow demonstrates how the **user's identity (sub)** is preserved through token exchange. The Keycloak setup script creates a demo user: `alice` (password: `alice123`)
...
| `scope` | `agent-authbridge-agent-aud profile email` | `auth-target-aud` | `agent-authbridge-agent-aud profile email` | `auth-target-aud` |
...
- The `agent-authbridge-agent-aud` scope adds the Agent's SPIFFE ID to all tokens' audience
...
**Fix:** Add the `agent-authbridge-agent-aud` scope to the caller client:
...
SCOPE_ID=$(curl -s -H "Authorization: Bearer $ADMIN_TOKEN" \
  ...
  jq -r ".[] | select(.name==\"agent-authbridge-agent-aud\") | .id")
...
echo "Added agent-authbridge-agent-aud scope to $CLIENT_ID"
'
```

**Note:** This is a security feature, not a limitation. The `agent-authbridge-agent-aud` scope adds the Agent's SPIFFE ID to the token's audience, authorizing the AuthProxy (using the same credentials) to exchange tokens.
